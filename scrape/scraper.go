...
func (a *async) ScrapeImage(i registryC.Image, repo *registryC.Repository) error {
	...
	if err != nil && err == store.ErrDoesNotExist {
		_, layers, err := a.CreateStoreImageFromRegistryImage(vp.Distinction(), i, repo)
		if err != nil {
			return err
		}
...
func (a *async) ScrapeLatestImage(i registryC.Image, repo *registryC.Repository) error {
	...
	if err != nil {
		if err == store.ErrDoesNotExist {
			...
			latestImage, latestImageLayers, err = a.CreateStoreImageFromRegistryImage(latestVP.Distinction(), latestRegImage, repo)
			if err != nil {
...
func (a *async) CreateStoreImageFromRegistryImage(distinction string, regImg registryC.Image, repo *registryC.Repository) (*store.Image, []*store.Layer, error) {
